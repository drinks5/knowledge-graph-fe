...
async function getClient(keyspace) {
...
const graknTx = getClient('social_network')
const baseTypes = {
  TYPE: 'META_TYPE',
 ...
 */
export const buildInstances = async (answers) => {
 ...
 * @param {ConceptMap[]} answers the untouched response of a transaction.query()
 */
export const buildTypes = async (answers) => {
 ...
 * @param {*} graknTx
 */
export const buildRPInstances = async (answers, currentData, shouldLimit, graknTx) => {
        ...
        if (concept.isRelation()) {
          ...
          promises.push(new Promise((resolve) => {
            relation.asRemote(graknTx).rolePlayersMap().then((rolePlayersMap) => {
              ...
              if (shouldLimit) rpEntries = rpEntries.slice(0, 30);
